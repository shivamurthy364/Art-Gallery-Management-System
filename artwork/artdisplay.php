...
$conn = mysqli_connect("localhost", "root", "", "art_gallery");
  ...
  $sql = "SELECT artid, title, year, type_of_art, price, eid, gid, artistid, image FROM artwork";
  ...
  if ($result->num_rows > 0)
   ...
   while($row = $result->fetch_assoc())
    {
    echo "<tr><td>" . $row["artid"]. "</td><td>" . $row["title"]. "</td><td>" . $row["year"]. "</td><td>" . $row["type_of_art"]. "</td><td>" . $row["price"]. "</td><td>" . $row["eid"]. "</td><td>" . $row["gid"]. "</td><td>". $row["artistid"]. "</td><td> <a href='$row[image]'><img src='". $row["image"]."' height='100' width='100'/></a></td></tr>";
    }
    echo "</table>";
